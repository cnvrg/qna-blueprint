...
class DataTrainingArguments:
    ...
    data_source: str = field(
        default=None,
        metadata={"help":"Provide the dataset file path"},
    )
...
def train_val_split(filepath):
    f= open(filepath,'r')
    data = json.load(f)
    train_percent = 0.90 #percentage of data to include in training
    total = len(data['data'])
    train_size = int(train_percent*total)
    train_examples = data['data'][:train_size]
    trainjson = {'data':train_examples}
    val_examples = data['data'][train_size:]
    valjson = {'data':val_examples}
    ...
    return train_size,total-train_size
...
def main():
    ...
    trainsplit,valsplit = train_val_split(data_args.data_source)
    ...
    sub1 = "train[:" + str(trainsplit-1) + "]"
    sub2 = "validation[:" + str(valsplit-1) + "]"

    train_dataset = nlp.load_dataset(data_args.dataset_path, name=data_args.qg_format, data_files='train.json', split=(str(sub1)) ,ignore_verifications=True)
    valid_dataset = nlp.load_dataset(data_args.dataset_path, name=data_args.qg_format, data_files='dev.json',split=(str(sub2)) ,ignore_verifications=True)
    ...
    if data_args.train_file_name is None:
        ...
        train_path = os.path.join(cnvrg_workdir, train_file_name)

        valid_file_name = f"valid_data_{data_args.task}_{data_args.qg_format}_{data_args.model_type}.pt"
        valid_path = os.path.join(cnvrg_workdir, valid_file_name)
    else:
        train_path = os.path.join(cnvrg_workdir, data_args.train_file_name)
        valid_path = os.path.join(cnvrg_workdir, data_args.valid_file_name)
    ...
    tokenizer_path=os.path.join(cnvrg_workdir,tokenizer_path)
